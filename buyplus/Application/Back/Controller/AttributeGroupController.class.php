...
class AttributeGroupController extends Controller
{
	...
	public function multiAction()
	{
		...
		switch ($operate) {
			case 'delete':
				$cond['attribute_group_id'] = ['in', $selected];
	...
	public function editAction()
	{
		...
		if (IS_POST) {
			...
			if ($model->validate($validate)->create()) {
				...
				$this->error('更新失败:'.$model->getError(), U('edit', ['attribute_group_id'=>I('post.attribute_group_id')]));
			}
	
		} else {
			// 获取当前数据
			$this->assign('row', $model->find($_GET['attribute_group_id']));
