...
<a href="<?php echo U('/userlist');?>" class="n1">UserList</a>
<!-- <a href="<?php echo U('/userlist');?>" class="n2">DelUser</a> --></h1>
<div class="bloglist left">
   <div id="gustbook" class="cate">
     	<div class="allcate">所有用户</div>
     	<table style="font-size:22px;width:700px;">
     	<tr style="color:red;"><td>注册邮箱</td><td>注册时间</td><td>删除</td></tr>
     	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr style="height:40px;"><td><?php echo ($row['email']); ?></td>
	     	<td><?php echo (date("Y-m-d",$row['add_time'])); ?></td>
	     	<td ><a style="color:blue;" href="<?php echo U('/userlist?id='.$row['user_id']);?>">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
     	</table>
   </div>
</div>
<!--right-->
