...
<div class="bloglist left">
   <div id="gustbook" class="cate">
     	...
     	<div class="catetitle">
     	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><span style="display:inline-block;
				width:80px;height:40px;line-height:40px;text-align:center;
				font-size:16px;color:white;background:#317EF3;margin:10px;
				border-radius:5px;"><?php echo ($row['cate_title']); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
     	</div>
     	<div class="insertcate">
     		<div class="one">添加分类</div>
     		<form class="two" action="<?php echo U('/catelist');?>" method="post">
     			<div>
     				分类名称：<input type="text" name="cate_title" class="three">
     			</div>	
     			<!-- <div>
     				父级分类：<select class="three">
     			...
     			<div>
     				<input type="submit" value="确认添加" class="four">
     			</div>
     		</form>
     	</div>
   </div>
