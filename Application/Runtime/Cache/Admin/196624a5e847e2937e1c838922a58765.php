...
<a href="<?php echo U('/article');?>" class="n1">ArticleList</a></h1>
<div class="bloglist left">
   <div id="gustbook" class="cate">
     	<div class="allcate">所有文章</div>
     	<table style="font-size:22px;width:700px;">
     	<tr style="color:#50A84D"><td>文章id</td><td>文章标题</td><td>发布时间</td><td>删除</td></tr>
     	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr style="height:40px;"><td><?php echo ($row['article_id']); ?></td><td style="width:400px;"><?php echo ($row['title']); ?></td>
	     	<td><?php echo (date("Y-m-d",$row['add_time'])); ?></td>
	     	<td><a style="color:blue;" href="<?php echo U('/article?id='.$row['article_id']);?>">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
     	</table>
   </div>
</div>
<!--right-->
